...
def hybrid_10_5_retrieval(backstory, metadata, faiss_index, entity_index):
    ...
    for claim in claims:
        ...
        seen = set()
        combined = []
        # Priority: Global action results first, then entity grounding results
        ...
        retrievals.append({
            ...
            "Search_type": f"Global(10)+Entity(5)",
...
for s in dataset:
    rets = hybrid_10_5_retrieval(s["user_input"], chunks, faiss_index, entity_index)
    verifs = []
    for r in rets:
...
print("-" * 50)
print(f"VERIFIED HYBRID 10+5 ACCURACY: {correct}/{total} = {correct/total * 100:.2f}%")

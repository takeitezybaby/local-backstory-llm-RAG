...
def make_prompt(claim, entity, evidence_list):
    ev_text = "\n".join([f"[{i+1}] {e['text']}" for i, e in enumerate(evidence_list)])
    return f"""You are an objective fact-checker evaluating a Claim against source excerpts from a novel.
...
Instructions:
- If the excerpts state facts confirming the claim, respond with "Verdict: SUPPORT".
- If the excerpts state facts that contradict or conflict with the claim (e.g. wrong parent, wrong job, false betrayal, opposite fate), respond with "Verdict: CONTRADICT".
- If the excerpts contain no information about the claim's events, respond with "Verdict: NOT MENTIONED".

Conclude with exactly one verdict line:
...
# 10 test claims (4 support, 3 contradict, 3 not mentioned)
test_claims = [
...
print("================================================================================")
print("       MULTI-MODEL NLI BENCHMARK COMPARISON (10 DIVERSE CLAIMS)                 ")
print("================================================================================")

models = ["phi3.5:latest", "mistral:7b"]
...
for model in models:
    ...
    for tc in test_claims:
        ...
        p = make_prompt(tc["claim"], ent, ev)
        resp = query_ollama(p, model)
    ...
    print(f"--> Accuracy for {model}: {acc:.1f}% ({correct}/{len(test_claims)})")
